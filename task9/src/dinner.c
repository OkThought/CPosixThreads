...
static int                   serve_the_table (Table *table, unsigned spaghetti_per_plate);
...
void                         int_ptr_swap (void **a, void **b);
void                         control_eating_priority (Table *table);

static const useconds_t      EAT_SPAGHETTI_PEACE_MICROSECONDS = 100000;
static const useconds_t      TIME_TO_THINK = 100000;
static const unsigned        AMOUNT_OF_SPAGHETTI_TO_EAT_AT_ONCE = 10;
...
int
serve_the_table (Table *table, unsigned spaghetti_per_plate) {
    int i;
    int code;
    ...
    for (i = 0; i < table->number_of_seats; ++i) {
        table->plates[i] = spaghetti_per_plate;
    }
...
WaiterControlTable (Table *table, Philosopher *philosophers, DinnerInvitation *dinnerInvitations,
    ...
    int code;
    code = serve_the_table (table, spaghettiPerPlate);
    if (code != SUCCESS) {
